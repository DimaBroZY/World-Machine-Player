...
func _ready() -> void:
	...
	if playlist.size() > 0:
		...
		curTrack.set_track_name(playlist[current_index].resource_path.get_file().get_basename())
	else:
...
func update_track_name() -> void:
	if playlist.size() > 0 and current_index < playlist.size():
		curTrack.set_track_name(track_names[current_index])  # Используем оригинальное имя
	else:
		curTrack.set_track_name(MUSIC_FILE.resource_path.get_file().get_basename())
